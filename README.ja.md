...
3. **インラインで指摘を追加** — 登場人物・概念データ・業務領域の **＋** をクリックすると
   小さなポップオーバーが開き、種類を選んでコメントを書き、*キューに追加* または
   *追加して実行* します。各指摘（finding）は3種類のいずれかで、処理が異なります:
...
4. **右のキューを処理** — 個別に実行するか、**キューを実行** で未処理をまとめて順に処理します。
   **Claudeセッションを継続する** をオンにすると指摘間で文脈を引き継ぎ、オフなら毎回新規
   セッションで実行します。指摘は `.dramaturgy/reviews.json` に永続化され、reframe後は
   プレビューが自動更新されます。
