...
1. **Claudeで一括初期化**（ヘッダーのボタン）— 全工程（解析 → 領域ツリー → 領域カード →
   ...
   部分的な地図が得られ後から補完できます。
2. **左のプレビューを読む** — 登場人物（先頭）、**概念データ**（物理テーブルを業務的意味で
   まとめたもの）と利用領域の対応、業務領域のボックス、そして **CRUD**（「領域から見る／
   概念データから見る」の双方向）を表示します。概念データには**システム固有のタグ**
   （例: マスタ／トランザクション）を付けられます。タグで絞り込めるほか、概念のタグセルの
   **＋** から直接編集できます（Claude不要）。タグ語彙はプロジェクトごとに
   `.dramaturgy/tags.json` に保持し、カード生成時に Claude へ渡されます。
3. **インラインで指摘を追加** — 登場人物・概念データ・業務領域の **＋** をクリックすると
   小さなポップオーバーが開き、種類を選んでコメントを書いて *キューに追加* します
