...
階層（parent/child/related）について（重要）:
- 領域の階層は領域ツリーが正本です。`parent_area_id` / `child_area_ids` /
  `related_area_ids` は、**分析パックの area に入っている値をそのままコピー**してください。
- **新しい領域IDを発明しないでください。** 領域ツリーに存在しない領域IDを child/parent/
  related に書いてはいけません（サブ機能を子領域として勝手に作らない）。パックに値が無ければ
  空配列／null のままにしてください。
