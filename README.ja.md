...
生成される地図は次を表示します: 業務領域はクリックで展開するボックスの一覧、
**概念データ**（物理テーブルを業務的意味でまとめたもの）と利用領域の対応、そして
**CRUD** を「領域から見る／概念データから見る」の双方向（同じ概念中心データを2方向で）。

5. **レビュー** — 登場人物・概念データ・業務領域を指して指摘を追加します。各指摘
   （*finding*）は3種類のいずれかで、処理が異なります:
   ...
   **Claudeセッションを継続する** をオンにすると指摘間で文脈を引き継ぎ、オフなら
   毎回新規セッションで実行します。指摘は `.dramaturgy/reviews.json` に永続化されます。
