...
1. **Claudeで一括初期化**（ヘッダーのボタン）— 全工程（解析 → 領域ツリー → 子領域レビュー →
   ...
   **解析指示設定** ボタンで、生成時のClaudeへの追加指示（例: 「概念をマスタ/トランザクションで
   タグ付けして」）を入力できます。指示はリポジトリごとに保存され
   （結果とは別の `.dramaturgy/init-instructions.txt`）、毎回の全体解析で再利用されます
   （毎回入力し直す必要はありません）。同じ場所で **Claudeの思考量（effort）** も選べます
   （low/medium/high/xhigh/max、既定は **xhigh**）。`.dramaturgy/init-effort.txt` に保存され、
   全体解析と各指摘の処理の両方に適用されます。
