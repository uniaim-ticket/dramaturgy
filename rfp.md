...
3. プレビュー内の各項目（登場人物・概念データ・業務領域）の **＋** から**インラインで指摘**を付ける。指摘はキューに積む（「追加」または「追加して実行」）。
4. 右のキューで、個別実行または**キュー一括実行**する。
...
* 指摘は `.dramaturgy/reviews.json` に永続化する（`{id, target_type, target_id,
  kind, comment, status, session_id, result, audit_result, proposal_ref}`）。
* 各指摘の実行時に、**Claudeセッションを継続するか新規にするか**を選べる
  （継続なら直近のレビューセッションを `--resume`）。
